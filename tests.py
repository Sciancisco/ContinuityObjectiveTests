...
def test_constraint(
    ...
    logging.info("Starting constraint tests...")
    for i, (x_init, u_init) in enumerate(zip(x_inits, u_inits)):
...
def test_objective(
    ...
    logging.info(f"Starting objective tests with weight={weight} and max_iteration_first={max_iteration_first}...")
    for i, (x_init, u_init) in enumerate(zip(x_inits, u_inits)):
        logging.info(f"Running test {i}...")
        start = time.time()
        ocp = prepare_ocp_first_pass(
        ...
        ocp = prepare_ocp_second_pass(biorbd_model_path, sol1, n_threads=n_threads)
        solver = Solver.IPOPT()
    ...
    logging.info(f"Done objective tests with weight={weight} and max_iteration_first={max_iteration_first}.")
